...
class A2DSentencesDataset(Dataset):
    ...
    def __init__(self, image_folder: Path, ann_file: Path, transforms, return_masks: bool,
        ...
        self.depth_dir = os.path.join(dataset_path, 'Release/depth')
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:
            ...
            video_depth_dir = os.path.join(self.depth_dir, video_id)
            ...
            # read frames
            imgs, labels, boxes, masks, valid, flows = [], [], [], [], [], []
            for j in range(self.num_frames):
                frame_indx = sample_indx[j]
                img = F.to_pil_image(video_frames[frame_indx].permute(2, 0, 1))
                imgs.append(img)
                flow = Image.open(os.path.join(video_depth_dir, '{}.png'.format(frame_indx)))
                flows.append(flow)
            ...
            mask_areas = area(mask_rles).astype(np.float_)
            f.close()
...
def make_coco_transforms(image_set, max_size=640):
    normalize = T.Compose([
        T.ToTensor(),
        T.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225], [0.522, 0.247, 0.257], [0.331, 0.244, 0.133])
    ])
...
if __name__ == '__main__':
    ...
    y = build('train', args)
    for i, (img, flow, target) in enumerate(y):
        print(i)
