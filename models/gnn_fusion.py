...
class MultimodalGNN(nn.Module):
    def __init__(self, feature_dim, num_relations, num_head, **kwargs):
        ...
        self.linear_src = nn.Linear(self.in_channels, self.out_channels)
        self.linear_dst = nn.Linear(self.in_channels, self.out_channels)
    ...
    def forward(self, x):  # x:[img+flow+word_length,h,w,c]
        ...
        for i in range(self.num_relations):
            edge_index_tmp = edge_index[:, edge_type == i]  # edges with same type
            if edge_index_tmp.numel() != 0:
                x_feat = self.relation_propagate_and_aggregation((x_feat, x_pos), edge_index_tmp,
                                                                 self.relation_cross_attention[i], device)

        x_feat, x_pos = self.graph_wise_propagation_and_aggregation((x_feat, x_pos), edge_index, edge_type, device)
        vision, motion = [], []
        for i in range(bs):
    ...
    #     node_features = alpha.unsqueeze(-1) * rearrange(x_feat, 'n c h w -> n c (h w)')[
    #         edge_index[0]]  # (edge_num,c,h*w)
    ...
    def relation_propagate_and_aggregation(self, x: Tuple[Tensor, Tensor],
        ...
        node_num, c, h, w = x_i.shape

        out = cross_attention(tgt=rearrange(x_i, 'n c h w -> (h w) n c'), memory=rearrange(x_j, 'n c h w -> (h w) n c'),
                              pos=rearrange(x_j_pos, 'n c h w -> (h w) n c'),
                              query_pos=None)

        out = rearrange(out, '(h w) n c -> n c h w', h=h)
        # 和原来直接相加
    ...
    def graph_wise_propagation_and_aggregation(self, x: Tuple[Tensor, Tensor],
        ...
        # 生成 edge_attr，size 与边数相关
        edge_attr = self.linear_edge(torch.ones(size=(edge_index.shape[1], 1), device=device))  # (edge_num, 256)

        # 计算 attention 权重
        alpha = (edge_attr @ self.weight)  # (edge_num, 256)
        alpha = softmax(src=alpha, index=edge_index[1], num_nodes=node_nums)  # 基于目标节点进行 softmax
...
def init_edge(img_per_batch: int, num_batches: int, device: str) -> Tuple[torch.Tensor, torch.Tensor]:
    edge_index = []
    edge_type = []
...
if __name__ == '__main__':
    edge_index, edge_type = init_edge(5, 1, 'cpu')
