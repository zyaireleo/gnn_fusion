...
class ReferFormer(nn.Module):
    ...
    def __init__(self, vision_backbone, motion_backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        Parameters:
            vision_backbone: torch module of the backbone to be used. See backbone.py
        ...
        if num_feature_levels > 1:
            # for vision backbone
            vision_backbone_outs = len(vision_backbone.strides[-3:])
            vision_proj_list = []
            for _ in range(vision_backbone_outs):
                in_channels = vision_backbone.num_channels[-3:][_]
                vision_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, hidden_dim, kernel_size=1),
                    nn.GroupNorm(32, hidden_dim),
                ))
            for _ in range(num_feature_levels - vision_backbone_outs):  # downsample 2x
                vision_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, hidden_dim, kernel_size=3, stride=2, padding=1),
                    nn.GroupNorm(32, hidden_dim),
                ))
                in_channels = hidden_dim
            self.vision_proj = nn.ModuleList(vision_proj_list)

            # for motion backbone
            motion_backbone_outs = len(motion_backbone.strides[-3:])
            motion_proj_list = []
            for _ in range(motion_backbone_outs):
                in_channels = motion_backbone.num_channels[-3:][_]
                motion_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, hidden_dim, kernel_size=1),
                    nn.GroupNorm(32, hidden_dim),
                ))
            for _ in range(num_feature_levels - motion_backbone_outs):  # downsample 2x
                motion_proj_list.append(nn.Sequential(
                    nn.Conv2d(in_channels, hidden_dim, kernel_size=3, stride=2, padding=1),
                    nn.GroupNorm(32, hidden_dim),
                ))
        ...
        self.vision_language = VisionLanguageFusionModule(d_model=hidden_dim, nhead=8)
        ...
        feature_channels = [self.vision_backbone.num_channels[0]] + 3 * [hidden_dim]
    ...
    def forward(self, samples, flows, captions, targets):
        ...
        visual_features, vision_position = self.vision_backbone(samples)
        motion_features, motion_position = self.motion_backbone(flows)

        b = len(captions)
        t = vision_position[0].shape[0] // b  # t=5
        ...
        if 'valid_indices' in targets[0]:
            valid_indices = torch.tensor([i * t + target['valid_indices'] for i, target in enumerate(targets)]).to(
                vision_position[0].device)
            for vf, mf in zip(visual_features, motion_features):
            ...
            for i, (vp, mp) in enumerate(zip(vision_position, motion_position)):
                vision_position[i] = vp.index_select(0, valid_indices)
                motion_position[i] = mp.index_select(0, valid_indices)
            samples.mask = samples.mask.index_select(0, valid_indices)
            # t: num_frames -> 1
            t = 1

        text_features, text_sentence_features = self.forward_text(captions, device=vision_position[0].device)
        ...
        for l, (vision_feat, vision_pos, motion_feat, motion_pos) in enumerate(
                zip(visual_features[-3:], vision_position[-3:], motion_features[-3:], motion_position[-3:])):
            vf, mask = vision_feat.decompose()
            vf = self.vision_proj[l](vf)
            mf, _ = motion_feat.decompose()
            mf_proj_l = self.motion_proj[l](mf)

            n, c, h, w = vf.shape  # (5,256,h,w)]

            # vision language early-fusion
            vf_proj_l = rearrange(vf, '(b t) c h w -> (t h w) b c', b=b, t=t, h=h, w=w)
            vision_pos = rearrange(vision_pos, '(b t) c h w -> (t h w) b c', b=b, t=t)
            vf_proj_l = self.vision_language(tgt=vf_proj_l,
                                             memory=text_word_features,  # (length,1,channel)
                                             memory_key_padding_mask=text_word_masks,
                                             pos=text_pos,
                                             query_pos=None
                                             )
            # gnn fusion
            vf_proj_l = rearrange(vf_proj_l, '(t h w) b c -> b t c h w', t=t, h=h, w=w)
            vision_pos = rearrange(vision_pos, '(t h w) b c -> b t c h w', t=t, h=h, w=w)
            text_word_features = rearrange(text_word_features, 'l b c ->b c l')
            text_pos = rearrange(text_pos, 'l b c ->b c l')

            mf_proj_l = rearrange(mf_proj_l, '(b t) c h w -> b t c h w', t=t)
            motion_pos = rearrange(motion_pos, '(b t) c h w -> b t c h w', t=t)
            vf_proj_l = self.modality_fusion(((vf_proj_l, vision_pos), (mf_proj_l, motion_pos),
                                              (text_word_features, text_pos)))[0]
            vision_pos = rearrange(vision_pos, 'b t c h w -> (b t) c h w', t=t)
            vision_srcs.append(vf_proj_l)
            masks.append(mask)
            vision_poses.append(vision_pos)
            text_word_features = rearrange(text_word_features, 'b c l ->l b c')
            text_pos = rearrange(text_pos, 'b c l ->l b c')
            assert mask is not None  # srcs:{torch.Size([5, 256, 48, 80]),torch.Size([5, 256, 24, 40]),torch.Size([5, 256, 12, 20])}
        if self.num_feature_levels > (len(visual_features) - 1):
            _len_srcs = len(visual_features) - 1  # fpn level
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    vf_proj_l = self.vision_proj[l](visual_features[-1].tensors)  # torch.Size([bt, c, h, w])
                    mf_proj_l = self.motion_proj[l](motion_features[-1].tensors)  # torch.Size([bt, 256, h, w])
                else:  # false
                    vf_proj_l = self.vision_proj[l](vision_srcs[-1])
                    mf_proj_l = self.motion_proj[l](motion_srcs[-1])
                m = samples.mask  # torch.Size([5, 384, 640])
                mask = F.interpolate(m[None].float(), size=vf_proj_l.shape[-2:]).to(torch.bool)[
                    0]  # torch.Size([5, 6, 10])
                vision_pos = self.vision_backbone[1](NestedTensor(vf_proj_l, mask)).to(
                    vf_proj_l.dtype)
                motion_pos = self.motion_backbone[1](NestedTensor(mf_proj_l, None)).to(
                    mf_proj_l.dtype)  # torch.Size([bs*t, c, h, w])

                # vision language early-fusion
                bt, c, h, w = vf_proj_l.shape
                vf = rearrange(vf_proj_l, '(b t) c h w -> (t h w) b c', b=b)
                vision_pos = rearrange(vision_pos, '(b t) c h w -> (t h w) b c', b=b, t=t)
                vision_feat = self.vision_language(tgt=vf,  # torch.Size([l, b, c])
                                                   memory=text_word_features,  # torch.Size([l, b, c])
                                                   memory_key_padding_mask=text_word_masks,  # torch.Size([b, l])
                                                   pos=text_pos,  # torch.Size([l, b, c])
                                                   query_pos=None
                                                   )
                # # gnn fusion
                vision_feat = rearrange(vision_feat, '(t h w) b c -> b t c h w', h=h, w=w)
                motion_feat = rearrange(mf_proj_l, '(b t) c h w -> b t c h w', b=b)
                vision_pos = rearrange(vision_pos, '(t h w) b c -> b t c h w', h=h, w=w)
                motion_pos = rearrange(motion_pos, '(b t) c h w -> b t c h w', b=b, t=t)
                text_word_features = rearrange(text_word_features, 'l b c ->b c l')
                text_pos = rearrange(text_pos, 'l b c ->b c l')
                vision_feat = self.modality_fusion(((vision_feat, vision_pos),
                                                    (motion_feat, motion_pos),
                                                    (text_word_features, text_pos)))[0]

                vision_pos = rearrange(vision_pos, 'b t c h w -> (b t) c h w', t=t)
                vision_srcs.append(vision_feat)
                masks.append(mask)
                vision_poses.append(vision_pos)
                text_word_features = rearrange(text_word_features, 'b c l ->l b c')
                text_pos = rearrange(text_pos, 'b c l ->l b c')
        ...
        hs, memory, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, inter_samples = \
                                            self.transformer(vision_srcs, text_embed, masks, vision_poses, query_embeds)
        ...
        for lvl in range(hs.shape[0]):
            ...
            if reference.shape[-1] == 4:
                ...
                tmp[..., :2] += reference
            outputs_coord = tmp.sigmoid()  # cxcy wh, range in [0,1]
        ...
        mask_features = self.pixel_decoder(visual_features, text_features, vision_position, memory,
...
class MLP(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim,
                 num_layers):  # input_dim 256  hidden_dim：256 output_dim:4 num_layers:3
        super().__init__()
